...
const Logo = () => {
  ...
  const springStyle = useSpring({
    transform: isHovering
      ? `rotate(${snowflakeAngle + 90}deg)`
      : `rotate(${snowflakeAngle}deg)`,
  });
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      setIsHovering(false);
      setSnowflakeAngle(snowflakeAngle + 90); // will this cause memory overflow if overused?
    }, timing);

    // avoid memory leak if component unmouts before timeout finishes
    return () => {
      window.clearTimeout(timeoutId);
    };
  }, [isHovering, timing, snowflakeAngle]);
